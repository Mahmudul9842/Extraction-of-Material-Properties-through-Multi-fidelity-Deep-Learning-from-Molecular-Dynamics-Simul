...
XL=load('T_phi_low_fidelity.txt');
XL=XL';
yL=load('v_low_fidelity.txt'); 
yL=yL';
filename1=sprintf('T_phi_high_fidelity.txt');
%filename1=sprintf('T_phi_high_fidelity.txt');
XH=load(filename1); %load input data
XH=XH';
XH1=load('T_phi_high_fidelity.txt');
XH1=XH1';
filename2=sprintf('v_high_fidelity.txt');
%filename2=sprintf('P_high_fidelity.txt');
yH=load(filename2); %load output data
yH=yH';
yH1=load('v_high_fidelity.txt');
yH1=yH1';
%y=[yL yH];
m=length(XH1(1,:));
%idx = randperm(m);
...
XH(2,:)=(XH(2,:)-min_phi)/(max_phi-min_phi);
XL(1,:)=(XL(1,:)-min_T)/(max_T-min_T);
XL(2,:)=(XL(2,:)-min_phi)/(max_phi-min_phi);
XH1(1,:)=(XH1(1,:)-min_T)/(max_T-min_T);
XH1(2,:)=(XH1(2,:)-min_phi)/(max_phi-min_phi);
...
filename3=sprintf('C:/15 Batch Thesis/Ridzy/brain/workspace_v.mat');
save(filename3)
